...
// const console.log = require('console.log')
// console.log.traffic = console.log.extend('traffic');
// console.log.error = console.log.extend('error*'); // '*' suffix means always visible
...
class BACnetDevice extends BACnetObject {
	...
	constructor(deviceInfo, hostInfo = {}) {
		...
		mqttClient.subscribe('point/pub/to', function (err) {
			// console.log()

		})

		// mqttClient.subscribe('point/pub/from', function (err) {
		// 	// console.log()
		...
		mqttClient.on('message', (topic, message) => {
			...
			try {
				...
				let mqttRes = { instance: this.instance, objInstance: data.objInstance, objType:  data.objType, priorityValue: priorityValue, priorityNum: priorityNum }
				mqttClient.publish('point/pub/from', JSON.stringify(mqttRes))
			} catch (error) {
				console.log(error)
			}
	...
	onUnhandledEvent(msg) {
		if (msg.header.expectingReply) {
			...
			console.log('Replying with error for unhandled service:', BE.getEnumName(enumType, msg.service));
	...
	onRegisterForeignDevice(msg) {
		console.log('[recv] Register foreign device:', msg.header.sender.address, 'for TTL', msg.payload.ttl);
		// Add to device table
	...
	onWhoIs(msg) {
		...
		if (
			...
			console.log(msg.header.sender, this.instance, this.getProperty(BE.PropertyIdentifier.SEGMENTATION_SUPPORTED).value, this.getProperty(BE.PropertyIdentifier.VENDOR_IDENTIFIER).value)

			console.log(`[send] Replying to whoIs(${msg.payload.lowLimit}..`
	...
	onWriteProperty(data) {
		...
		if (data.payload.value.property.index === 0xFFFFFFFF) {
			...
			if (!slot) return this.client.errorResponse(data.header.sender, data.service, data.invokeId, bacnet.enum.ErrorClass.PROPERTY, bacnet.enum.ErrorCode.INVALID_ARRAY_INDEX);
			console.log(6666)
	...
	onReadProperty(msg) {
		...
		console.log(`[recv/${msg.header.sender.address}] readProperty: ${objectIdName}/${propertyName}`);

		const object = this.getObject(msg.payload.objectId.instance, msg.payload.objectId.type);
		if (!object) {
			console.log(`[send/${msg.header.sender.address}] Requested non-existent `
		...
		if (msg.payload.property.index === 0xFFFFFFFF) {
			...
			console.log(`[send/${msg.header.sender.address}] readPropertyResponse@all: ${objectIdName}/${propertyName} => %O`, content);
		} else {
			...
			console.log(`[send/${msg.header.sender.address}] readPropertyResponse@${msg.payload.property.index}: ${objectIdName}/${propertyName}`);
		}
	}
	...
	onReadPropertyMultiple(msg) {
		...
		console.log(JSON.stringify(msg))
		console.log(`[recv/${msg.header.sender.address}] readPropertyMultiple`);
		const responseList = [];
		...
		propGroups.forEach(propGroup => {
			...
			console.log(`[recv/${msg.header.sender.address}] readPropertyMultiple: object ${objectIdName}/%o`, propListFriendly);
			...
			console.log(`[BE.ObjectTypesSupported.DEVICE ${propGroup.objectId.instance}] readPropertyMultiple: instance ${BE.ASN1_MAX_PROPERTY_ID}`);
			if ((propGroup.objectId.type === BE.ObjectTypesSupported.DEVICE) && (propGroup.objectId.instance === BE.ASN1_MAX_PROPERTY_ID)) {
	...
	onSubscribeCov(msg) {
		...
		console.log(`[recv/${msg.header.sender.address}] subscribeCov: object ${objectIdName}`);

		const object = this.getObject(msg.payload.monitoredObjectId.instance, msg.payload.monitoredObjectId.type);
		if (!object) {
			// Invalid object
			console.log(`${msg.header.sender.address} tried to subscribe to non-existent object ${objectIdName}`);
			console.log(`[send/${msg.header.sender.address}] Returning error OBJECT:UNKNOWN_OBJECT`);
			this.client.errorResponse(
				msg.header.sender,
				msg.service,
				msg.invokeId,
		...
		if (msg.payload.lifetime) {
			...
			console.log(`Adding subscription to object ${objectIdName} for ${msg.header.sender.address}#${msg.payload.subscriberProcessId}`);
			...
			console.log(`Removing subscription to object ${objectIdName} by ${msg.header.sender.address}#${msg.payload.subscriberProcessId}`);
		}

		console.log(`[send/${msg.header.sender.address}] simpleAckResponse for subscribeCov`);
	...
	onSubscribeProperty(msg) {
		console.log("onSubscribeProperty")
		const typeName = Util.getEnumName(BE.ObjectType, msg.payload.monitoredObjectId.type);
		const objectIdName = typeName + ':' + msg.payload.monitoredObjectId.instance;
		console.log(`[recv/${msg.header.sender.address}] subscribeCovProperty: object ${objectIdName}`);

		if (!msg.payload.issueConfirmedNotifications) {
			// Unconfirmed notifications aren't yet implemented in bacstack
			console.log(`Returning error to ${msg.header.sender.address} - unable to subscribe with unconfirmed notifications`);
			console.log(`[send/${msg.header.sender.address}] Returning error OBJECT:OPTIONAL_FUNCTIONALITY_NOT_SUPPORTED`);
			this.client.errorResponse(
				msg.header.sender,
				msg.service,
				msg.invokeId,
		...
		if (!property) {
			// Invalid property
			console.log(`${msg.header.sender.address} tried to subscribe to non-existent property ${objectIdName}/${propertyName}`);
			console.log(`[send/${msg.header.sender.address}] Returning error PROPERTY:UNKNOWN_PROPERTY`);
			this.client.errorResponse(
				msg.header.sender,
				msg.service,
				msg.invokeId,
		...
		if (msg.payload.lifetime) {
			...
			console.log(`Adding subscription to property ${objectIdName}/${propertyName} for ${msg.header.sender.address}#${msg.payload.subscriberProcessId}`);
			...
			console.log(`Removing subscription to ${objectIdName}/${propertyName} by ${msg.header.sender.address}#${msg.payload.subscriberProcessId}`);
		}

		console.log(`[send/${msg.header.sender.address}] simpleAckResponse for subscribeCovProperty`);
		this.client.simpleAckResponse(
			msg.header.sender,
			msg.service,
			msg.invokeId,
	...
	onObjectPropertyChanged(object, property) {
		// console.log("onObjectPropertyChanged")
		let propList = [];
		...
		if (property.subscriptions) {
			property.subscriptions.forEach(sub => {
				...
				console.log(`[send/${sub.subscriberAddress.address}] confirmedCOVNotification: property ${objectIdName}/${propertyName}`);
				this.sendPropertyCov(sub, propList);
			});
		}
	}
	...
	sendPropertyCov(sub, propList) {
		console.log("sendPropertyCov")
		const typeName = Util.getEnumName(BE.ObjectType, sub.monitoredObjectId.type);
		const objectIdName = typeName + ':' + sub.monitoredObjectId.instance;
		console.log(`[send/${sub.subscriberAddress.address}] confirmedCOVNotification: object ${objectIdName}, ${propList.length} items`);
		if (sub.issueConfirmedNotifications) {
			this.client.confirmedCOVNotification(
				...
				err => {
					if (err) {
						console.log(`[recv/${sub.subscriberAddress.address}] confirmedCOVNotification was rejected:`, err);
					} else {
						console.log(`[recv/${sub.subscriberAddress.address}] confirmedCOVNotification was ACKed`);
					}
				}
			);
		} else {
				...
				value: (() => {
					switch (property.typeId) {
						...
						default:
							...
							console.log(`No encoding for data type ${typeName}`);
